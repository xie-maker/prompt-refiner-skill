---
name: prompt-refiner
description: Refine a user's rough request into a clear, outcome-first executable prompt and then complete the task. Use when the user explicitly invokes $prompt-refiner or asks to first optimize, polish, structure, or rewrite their request as a better AI prompt before execution. Use Pro Mode when the invocation includes an independent -pro flag for deep diagnosis, prompt review, failure analysis, comparison, test cases, or stronger prompt engineering. Applies to writing, planning, coding, courseware, research, analysis, copywriting, spreadsheets, and project planning.
---

# Prompt Refiner

Use this skill as a prompt refinement pass before doing the user's task. Preserve the user's intent, clarify the execution contract, then act on it. Favor outcome-first prompts: define what good looks like, evidence boundaries, constraints, and stopping conditions; avoid prescribing unnecessary step-by-step process.
...
- **Pro Mode**: Use when the user message contains an independent `-pro` flag, such as `$prompt-refiner -pro improve this prompt`. Treat `-pro` as a control flag, not as part of the user's raw request or final optimized prompt.
...
- Keep `allow_implicit_invocation: false` behavior: this skill should run only when explicitly invoked or clearly requested.
...
5. Execute the task using the optimized prompt.
...
7. For long or tool-heavy tasks, give the user a brief visible update before doing hidden work.
...
For extra task-pattern guidance in Pro Mode, read `references/prompt-patterns.md` only when it would improve the review.
...
- Do not add vague motivational persona text.
...
- For code or file tasks, follow normal repository inspection, editing, and verification practices after the prompt refinement.
